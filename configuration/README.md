...
> **SOPS workflows moved.** `create-secrets-file` now lives in the
> [`secrets`](../secrets/README.md) module as `render-template`. Replace
> `dagger call -m configuration create-secrets-file ...` with
> `dagger call -m secrets render-template ...` (all flags carry over unchanged).
