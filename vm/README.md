...
- **The wait fails fast on what waiting cannot fix.** A kubeconfig that names an unreachable server, a missing context, rejected credentials or a cluster with no KubeVirt API all abort on the first poll instead of burning `--wait-timeout` — the kubernetes module returns no exit code (every command is wrapped in `|| true`), so these are recognised from kubectl's own wording, and anything unrecognised stays retryable. A genuine `NotFound` is the one error worth waiting through.
- **The VMI has its own, shorter deadline.** `--vmi-appear-timeout` (default 120s) bounds how long the VMI object may take to exist at all; `--wait-timeout` then bounds how long its IP may take. A VM that KubeVirt never instantiates — a pending PVC, a missing image, an unschedulable node — therefore fails in two minutes rather than fifteen. Pass `0` to fold the check back into `--wait-timeout`.
